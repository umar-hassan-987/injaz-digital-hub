...
const FooterLinkColumn = ({ title, links }) => (
  <div className="flex flex-col gap-6">
    <h4 className="text-[11px] font-black uppercase tracking-[0.2em] text-[#0F5F6A]">{title}</h4>
    <ul className="flex flex-col gap-4">
      {links.map((link, idx) => (
        <li key={idx}>
          <Link to={link.path} className="text-[15px] font-medium text-gray-600 hover:text-[#0F5F6A] transition-colors flex items-center group gap-2 w-max">
            {link.name}
            <ArrowUpRight size={14} className="opacity-0 -translate-y-1 translate-x-1 group-hover:opacity-100 group-hover:translate-y-0 group-hover:translate-x-0 transition-all text-[#0F5F6A]" />
          </Link>
        </li>
      ))}
    </ul>
      ...
      <div className="max-w-[1400px] mx-auto px-6 sm:px-10 lg:px-24 xl:px-32 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-12 mb-20">
          ...
          <div className="lg:col-span-4">
            <h4 className="text-[11px] font-black uppercase tracking-[0.2em] text-[#0F5F6A] mb-6">Headquarters</h4>
            <div className="space-y-6">
              <a href="https://maps.google.com/?q=Office+No+3+First+Floor+Islamabad+Arcade,+G-11+Markaz+Islamabad" target="_blank" rel="noopener noreferrer" className="flex items-start gap-4 group">
                <div className="w-12 h-12 rounded-2xl bg-white border border-gray-200 shadow-sm flex items-center justify-center text-[#0F5F6A] shrink-0 group-hover:bg-[#0F5F6A]/10 transition-colors">
                  <MapPin size={20} strokeWidth={1.5} />
                </div>
                <div className="pt-1">
                  <p className="text-[15px] font-medium text-gray-600 leading-relaxed group-hover:text-[#0F5F6A] transition-colors">{companyInfo.address}</p>
                </div>
              </a>
              <a href={`tel:${companyInfo.phone.replace(/\s+/g, '')}`} className="flex items-center gap-4 group">
                ...
                <p className="text-[15px] font-medium text-gray-600 group-hover:text-[#0F5F6A] transition-colors">{companyInfo.phone}</p>
              </a>
              <a href={`mailto:${companyInfo.email}`} className="flex items-center gap-4 group">
                <div className="w-12 h-12 rounded-2xl bg-white border border-gray-200 shadow-sm flex items-center justify-center text-[#0F5F6A] shrink-0 group-hover:bg-[#0F5F6A]/10 transition-colors">
                  <Mail size={20} strokeWidth={1.5} />
                </div>
                <p className="text-[15px] font-medium text-gray-600 group-hover:text-[#0F5F6A] transition-colors">{companyInfo.email}</p>
              </a>
            </div>
          </div>
        </div>
        ...
        <div className="pt-8 border-t border-gray-200 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-8">
            <Link to="/privacy-policy" className="text-xs font-bold text-gray-500 hover:text-[#0F5F6A] tracking-wider transition-colors">PRIVACY POLICY</Link>
            <Link to="/terms-of-service" className="text-xs font-bold text-gray-500 hover:text-[#0F5F6A] tracking-wider transition-colors">TERMS OF SERVICE</Link>
          </div>
        </div>
      </div>
    </footer>
