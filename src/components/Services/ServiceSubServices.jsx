      ...
      <div className="max-w-[1400px] mx-auto px-6 sm:px-10 lg:px-24 xl:px-32 relative z-10 w-full">
        <div className="max-w-2xl mb-16 lg:mb-24">
          <div className="flex items-center gap-4 mb-4">
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.4em] text-accent">
              {t('serviceDetail.subServices.subtitle', 'Specializations')}
            </span>
          </div>
          <h2 className={`text-3xl lg:text-5xl font-bold font-display tracking-tight leading-tight text-gray-900`}>
            {t('serviceDetail.subServices.titlePart1', 'Granular')} <br />
            <span className="text-accent italic font-light">{t('serviceDetail.subServices.titleHighlight', 'Expertise Areas')}</span>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
          {subServices.map((sub, idx) => (
            ...
            >
              <h3 className={`text-2xl font-bold font-display mb-6 text-gray-900 ${isRTL ? 'text-right' : 'text-left'}`}>
                {t(`servicesDetail.${service.slug}.subServices.${idx}.title`, sub.title)}
              </h3>
              <p className={`text-base font-medium leading-relaxed mb-8 text-gray-600 ${isRTL ? 'text-right' : 'text-left'}`}>
                {t(`servicesDetail.${service.slug}.subServices.${idx}.description`, sub.description)}
              </p>
                ...
                {sub.points?.map((point, i) => (
                  <li key={i} className="flex items-start gap-3">
                    ...
                    <span className={`text-sm font-bold tracking-tight text-gray-700`}>
                      {t(`servicesDetail.${service.slug}.subServices.${idx}.points.${i}`, point)}
                    </span>
                  </li>
                ))}
              </ul>
